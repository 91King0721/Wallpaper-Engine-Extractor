...
> **提示：** 只有壁纸类型为 **「视频」** 的场景壁纸才能提取出完整的视频文件（MP4）。其他类型的场景壁纸（如 2D/3D 渲染场景、Web 场景等）可能只会提取到图片、音频、着色器和元数据，而不会产生独立的视频文件。
...
如果提取出的 ideo/ 中有 MP4 文件（60 FPS 动态壁纸常用），需要额外截取一帧：
...
|------|------|
| src/gui.py | 图形界面程序 |
| src/run.py | 命令行一键提取脚本 |
| src/extractor.py | 手动分步提取 CLI |
| src/extractor/ | 提取核心模块 |
